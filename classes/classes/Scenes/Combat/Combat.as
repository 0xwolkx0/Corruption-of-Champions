...
public class Combat extends BaseContent {
	...
	public var isBowDamage:Boolean = false; // switch for random damage calculation
...
public function combatMenu(newRound:Boolean = true):void { //If returning from a sub menu set newRound to false
	...
	MDODialogs = false; // JA dialogs, look 3875
	MDOCount = 0;
	MSGControll = false;
	MSGControllForEvasion = false;
	var isBowDamage:Boolean = false;
	}
...
public function fireBow():void {
	if (flags[kFLAGS.MELEE_DAMAGE_OVERHAUL] == 1) isBowDamage = true;
	clearOutput();
...
//	}
	...
	if ((player.hasStatusEffect(StatusEffects.Blind) && rand(2) == 0) || (monster.spe - player.spe > 0 && int(Math.random() * (((monster.spe - player.spe) / 4) + 80)) > 80)) {
		...
		if(monster.short == "Akbal") outputText("Akbal moves like lightning, weaving in and out of your furious strikes with the speed and grace befitting his jaguar body.\n");
			...
			if ((MDOCount == maxCurrentAttacks) && (MSGControllForEvasion == true) && (MSGControll == false)) {
...
public function DamageOverhaul(damage:Number):Number {
	...
	// threshold math
	if (isBowDamage) lower_threshold = 25, higher_threshold = 125; // Bow damage would have higher difference than melee.
	...
	if ((lower_threshold < 15) && (!isBowDamage)) { // Lowest possible value for melee
		lower_threshold = 15;  
	if ((lower_threshold < 10) && (isBowDamage)) // Lowest possible value for bow
		lower_threshold = 10;
	}
	...
	for (var i:int = 1; i < times; i++) // rolling higher / lower value
	{
		...
		if ((MDODialogs) && (MDOCount == 1)) {
				if (lock) {
					...
					if ((random_value > 20) && (random_value <= 40)) outputText("You hit [monster a] [monster name], but due to deft movements of your foe your strike lacks of power. ");
